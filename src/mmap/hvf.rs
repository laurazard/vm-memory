...
mod hypervisor {
    ...
    // See hv/hv_kern_types.h for the definition of hv_return_t.
    #[derive(Debug, PartialEq, Eq)]
    #[repr(i64)]
    pub enum hv_return_t {
        HV_SUCCESS             = 0,
        HV_ERROR               = 0xfae94001,
        HV_BUSY                = 0xfae94002,
    ...
    impl std::fmt::Display for hv_return_t {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            match self {
                hv_return_t::HV_SUCCESS => write!(f, "HV_SUCCESS"),
                hv_return_t::HV_ERROR => write!(f, "HV_ERROR"),
                hv_return_t::HV_BUSY => write!(f, "HV_BUSY"),
                hv_return_t::HV_BAD_ARGUMENT => write!(f, "HV_BAD_ARGUMENT"),
                hv_return_t::HV_ILLEGAL_GUEST_STATE => write!(f, "HV_ILLEGAL_GUEST_STATE"),
                hv_return_t::HV_NO_RESOURCES => write!(f, "HV_NO_RESOURCES"),
                hv_return_t::HV_NO_DEVICE => write!(f, "HV_NO_DEVICE"),
                hv_return_t::HV_DENIED => write!(f, "HV_DENIED"),
                hv_return_t::HV_UNSUPPORTED => write!(f, "HV_UNSUPPORTED"),
            }
        }
    }
    ...
    pub const HV_ALLOCATE_DEFAULT: hv_memory_flags_t = 0;
...
pub enum Error {
    ...
    SeekStart(io::Error),
    /// The hypervisor framework call returned an error.
    #[error("Hypervisor framework error: {0}")]
    Hypervisor(hypervisor::hv_return_t),
...
impl<B: Bitmap> MmapRegionBuilder<B> {
    ...
    fn build_anonymous(self) -> Result<MmapRegion<B>> {
        ...
        eprintln!("=== HVF Memory Allocation Backtrace ===");
        eprintln!("{}", bt);
        eprintln!("=====================================");

        // hv_vm_allocate doesn't support any flags. Ignore them.
        let mut allocated_ptr: *mut std::os::raw::c_void = std::ptr::null_mut();
        let alloc_result = unsafe { hypervisor::hv_vm_allocate(&mut allocated_ptr, self.size, hypervisor::HV_ALLOCATE_DEFAULT) };

        if alloc_result != hypervisor::hv_return_t::HV_SUCCESS {
            return Err(Error::Hypervisor(alloc_result));
        }

        Ok(MmapRegion {
            addr: allocated_ptr as *mut u8,
            size: self.size,
            bitmap: self.bitmap,
